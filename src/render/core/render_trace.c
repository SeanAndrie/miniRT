...
static bool	fill_hit(t_ray *ray, t_isect *is, t_hit *hit)
{
	...
	else if (is->obj->type == OBJ_PLANE)
	{
		hit->normal = normal_plane(ray->dir, &is->obj->data.plane);
		hit->rgb = is->obj->data.plane.rgb;
	}
	else if (is->obj->type == OBJ_CYLINDER)
	{
		hit->normal = normal_cylinder(hit->point, &is->obj->data.cylinder,
				hit->t);
		hit->rgb = is->obj->data.cylinder.rgb;
	}
	else if (is->obj->type == OBJ_CONE)
	{
		hit->normal = normal_cone(hit->point, &is->obj->data.cone, hit->t);
		hit->rgb = is->obj->data.cone.rgb;
	}
	return (true);
}
