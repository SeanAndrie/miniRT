...
t_bool					render_threaded(t_pool *pool, t_display *disp,
							t_scene *scene);
t_bool					render_init(t_pool *pool, t_display *disp,
							t_scene *scene);
t_bool					render_trace(t_ray ray, t_hit *hit, t_scene *scene);

float					isect_obj(t_ray *ray, t_surface *hit_loc,
							t_object *obj);
...
t_ray					ray_create(t_vec3 orig, const t_tensr *rdir, int x,
							int y);
...
t_bool					tile_create(t_tile *tile, t_tensr *buffer,
							t_tensr *rdir, t_tile_map *tm);
void					tile_free(t_tile *tile, size_t n);

t_bool					pool_init(t_pool *pool, t_display *disp,
							t_scene *scene);
